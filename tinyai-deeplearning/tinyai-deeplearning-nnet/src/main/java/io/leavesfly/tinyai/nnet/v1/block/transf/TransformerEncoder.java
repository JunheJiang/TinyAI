package io.leavesfly.tinyai.nnet.v1.block.transf;
...
public class TransformerEncoder extends Encoder {
    ...
    private List<TransformerEncoderBlock> encoderLayers;
    ...
    public void init() {
        if (!alreadyInit) {
            ...
            for (int i = 0; i < numLayers; i++) {
                TransformerEncoderBlock layer = new TransformerEncoderBlock(
    ...
    public Variable layerForward(Variable... inputs) {
        ...
        for (TransformerEncoderBlock layer : encoderLayers) {
     ...
     * 获取编码器层列表
     */
    public List<TransformerEncoderBlock> getEncoderLayers() {
     ...
     * 获取指定索引的编码器层
     */
    public TransformerEncoderBlock getEncoderLayer(int index) {
